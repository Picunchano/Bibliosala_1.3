...
from data_manager import cargar_reservas, play_sound
...
class AnalisisWindow(tk.Toplevel):
    ...
    def _realizar_analisis(self):
        ...
        self._analizar_tipo_prestamo(filtered_reservas) # Nuevo análisis
    ...
    def _filtrar_reservas(self):
        ...
        for reserva in self.reservas:
            try:
                ...
                print(f"Advertencia: No se pudo parsear la fecha de la reserva o falta una clave: {e}. Datos: {reserva}")
                continue
        return filtered
    ...
    def _analizar_horarios_actividad(self, filtered_reservas):
        ...
        for reserva in filtered_reservas:
            try:
                hora_inicio = datetime.datetime.strptime(reserva['HoraInicio'], "%H:%M").time()
                hora_fin = datetime.datetime.strptime(reserva['HoraFin'], "%H:%M").time()
                ...
                print(f"Advertencia: No se pudo parsear la hora de la reserva o falta una clave: {e}. Datos: {reserva}")
                continue
